echo "Ensure bogus ICMP responses are ignored"
	...
	temp1=`grep 'net.ipv4.icmp_ignore_bogus_error_responses' /etc/sysctl.conf | wc -l`
	if [ $temp1 -eq 0 ];then
		echo "net.ipv4.icmp_ignore_bogus_error_responses = 1" >> /etc/sysctl.conf
		check1=$?

	else
		target1=`grep -n '^net.ipv4.icmp_ignore_bogus_error_responses' | cut -f1 -d:`
		sed -i ${target1}'s/.*/net.ipv4.icmp_ignore_bogus_error_responses = 1/' /etc/sysctl.conf
		check1=$?
