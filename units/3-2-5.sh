echo "Ensure broadcast ICMP requests are ignored"
	...
	temp1=`grep 'net.ipv4.icmp_echo_ignore_broadcasts' /etc/sysctl.conf | wc -l`
	if [ $temp1 -eq 0 ];then
		echo "net.ipv4.icmp_echo_ignore_broadcasts = 1" >> /etc/sysctl.conf
		check1=$?
	else
		$target1=`grep -n 'net.ipv4.icmp_echo_ignore_broadcasts' /etc/sysctl.conf | cut -f1 -d:`
		sed -i ${target1}'s/.*/net.ipv4.icmp_echo_ignore_broadcasts = 1/' /etc/sysctl.conf
		check1=$?
