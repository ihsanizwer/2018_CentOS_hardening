echo "Ensure IPv6 router advertisements are not accepted"
	...
	temp1=`grep '^net.ipv6.conf.all.accept_ra' /etc/sysctl.conf | wc -l `
	temp2=`grep '^net.ipv6.conf.default.accept_ra' /etc/sysctl.conf | wc -l `
	...
	if ([ $temp1 -eq 0 ] && [ $temp2 -eq 0 ]);then
		...
		target1=`grep -n '^net.ipv6.conf.all.accept_ra' /etc/sysctl.conf | cut -f1 -d:`
		target2=`grep -n '^net.ipv6.conf.default.accept_ra' /etc/sysctl.conf | cut -f1 -d:`
		sed -i ${target1}'s/.*/net.ipv6.conf.all.accept_ra = 0/' /etc/sysctl.conf
		check1=$?
		sed -i ${target2}'s/.*/net.ipv6.conf.default.accept_ra = 0/' /etc/sysctl.conf
		check2=$?
