cho "Ensure suspicious packets are logged"
	...
	temp1=`grep '^net.ipv4.conf.all.log_martians' /etc/sysctl.conf | wc -l`
	temp2=`grep '^net.ipv4.conf.default.log_martians' /etc/sysctl.conf | wc -l`
	if ([ $temp1 -eq 0 ] && [ $temp2 -eq 0 ]);then
		...
		target1=`grep -n '^net.ipv4.conf.all.log_martians' /etc/sysctl.conf | cut -f1 -d:`
		target2=`grep -n '^net.ipv4.conf.default.log_martians' /etc/sysctl.conf | cut -f1 -d:`
		sed -i ${target1}'s/.*/net.ipv4.conf.all.log_martians = 1/' /etc/sysctl.conf
		check1=$?
		sed -i ${target2}'s/.*/net.ipv4.conf.default.log_martians = 1/' /etc/sysctl.conf
		check2=$?
